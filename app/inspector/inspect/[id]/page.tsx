...
export default function InspectionPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <section className="premium-panel mb-6 grid gap-4 border border-[#407F3E]/15 bg-gradient-to-br from-white via-[#f5fbf4] to-[#edf7eb] p-5 lg:grid-cols-[1.35fr_repeat(3,minmax(0,1fr))]">
        <div className="space-y-3">
          <span className="eyebrow-chip">{language === 'vi' ? 'Evidence workflow' : 'Evidence workflow'}</span>
          <div>
            <h2 className="text-xl font-semibold tracking-tight" style={{ fontFamily: 'var(--font-archivo)' }}>
              {language === 'vi' ? 'Moi buoc kiem dinh can nhin nhu mot quy trinh tao bang chung cho giao dich.' : 'Every inspection step should feel like evidence production for a live transaction.'}
            </h2>
            <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
              {language === 'vi'
                ? 'Them lop tom tat nay de page inspect trong nghiem hon, co chat cong cu nghiep vu thay vi mot form checklist thong thuong.'
                : 'This overview makes the inspection page feel more like a serious field tool than a standard checklist form.'}
            </p>
          </div>
        </div>
        <div className="premium-subpanel space-y-1">
          <p className="metric-label">{language === 'vi' ? 'Checks locked' : 'Checks locked'}</p>
          <p className="metric-value">{checkedCount}/{checklistItems.length}</p>
          <p className="text-xs text-muted-foreground">{language === 'vi' ? 'hang muc da duoc gan status ro rang' : 'inspection lines already assigned a clear status'}</p>
        </div>
        <div className="premium-subpanel space-y-1">
          <p className="metric-label">{language === 'vi' ? 'Evidence score' : 'Evidence score'}</p>
          <p className="metric-value">{evidenceCompleteness}%</p>
          <p className="text-xs text-muted-foreground">{language === 'vi' ? 'do day du cua note, anh va ket qua de ra report' : 'how complete the notes, photos, and statuses are for report handoff'}</p>
        </div>
        <div className="premium-subpanel space-y-1">
          <p className="metric-label">{language === 'vi' ? 'Projected grade' : 'Projected grade'}</p>
          <p className="metric-value">{calculateGrade()}</p>
          <p className="text-xs text-muted-foreground">{language === 'vi' ? 'grade tam thoi giup doc nhanh commercial impact cua report' : 'a live grade that previews the commercial impact of the report'}</p>
        </div>
      </section>
      {/* Seller Info */}
