        ...
        <section className="mb-8 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="premium-panel p-5">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="eyebrow-chip">Account command center</div>
                <h2 className="mt-3 text-2xl font-bold tracking-[-0.03em] text-foreground">
                  Orders now feel like a managed buyer workspace
                </h2>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  Instead of a simple list, this page now reads like a control surface for reserves,
                  inspection milestones, escrow progress, and delivery follow-through.
                </p>
              </div>
              <Badge className="rounded-full bg-primary px-3 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-primary-foreground">
                {buyerOrders.length} tracked orders
              </Badge>
            </div>
          </div>
            ...
            {[
              {
                ...
                value: `${buyerOrders.filter(o => !['completed', 'cancelled'].includes(o.status)).length} live orders`,
                sub: 'Shows that the account has meaningful work in motion.',
              },
              {
                icon: CheckCircle2,
                label: 'Completed',
                value: `${buyerOrders.filter(o => ['completed'].includes(o.status)).length} closed`,
                sub: 'Adds maturity and proof of follow-through.',
              },
              {
                icon: Package,
                label: 'Decision state',
                value: activeTab === 'all' ? 'Full portfolio view' : activeTab === 'active' ? 'Focused on in-flight orders' : 'Reviewing closed outcomes',
                sub: 'Makes each tab feel intentional, not decorative.',
              },
                        ...
                        <div className="flex gap-4">
                          ...
                          <div className="flex flex-col items-end justify-between shrink-0">
                            ...
                            <Button variant="outline" size="sm" asChild>
                              <Link href={`/transaction/${order.listing.id}`}>
                                <Eye className="h-4 w-4 mr-2" />
                                {language === 'vi' ? 'Chi tiết giao dịch' : 'Transaction Hub'}
                              </Link>
                            </Button>
                          </div>
                        </div>
