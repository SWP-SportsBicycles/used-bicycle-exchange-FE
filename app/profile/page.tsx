        ...
        <section className="mb-8 grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="premium-panel p-5">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="eyebrow-chip">Identity and trust profile</div>
                <h2 className="mt-3 text-2xl font-bold tracking-[-0.03em] text-foreground">
                  Account details now support the marketplace story better
                </h2>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  This page is framed less like a plain settings form and more like a trust layer for the user:
                  identity, role, recent activity, and profile quality live in one place.
                </p>
              </div>
            ...
            {[
              {
                icon: User,
                label: 'Profile state',
                value: isEditing ? 'Editing live details' : 'Profile ready',
                sub: 'Shows if the user is maintaining their account identity.',
              },
              {
                icon: Clock,
                label: 'Recent activity',
                value: transactions.length > 0 ? `${transactions.length} recent records` : 'No recent transactions',
                sub: 'Connects the profile to marketplace usage, not just settings.',
              },
              {
                icon: Shield,
                label: 'Trust role',
                value: user.role === 'seller' ? 'Commercial seller profile' : user.role === 'inspector' ? 'Inspection operator profile' : 'Buyer identity profile',
                sub: 'Makes the account page adapt to the user role more clearly.',
              },
