...
export default function DisputesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="premium-panel grid gap-4 border border-destructive/15 bg-gradient-to-br from-white via-[#fff7f5] to-[#fff1ee] p-5 lg:grid-cols-[1.35fr_repeat(3,minmax(0,1fr))]">
        <div className="space-y-3">
          <span className="eyebrow-chip">{language === 'vi' ? 'Resolution desk' : 'Resolution desk'}</span>
          <div>
            <h2 className="text-xl font-semibold tracking-tight" style={{ fontFamily: 'var(--font-archivo)' }}>
              {language === 'vi' ? 'Xu ly dispute nhu mot quy trinh quan tri trust, khong chi la ticket queue.' : 'Handle disputes like a trust-governance workflow, not just a ticket queue.'}
            </h2>
            <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
              {language === 'vi'
                ? 'Overview nay giup trang trong nghiem hon va doc ro pressure cua cac case dang mo truoc khi admin lao vao chi tiet.'
                : 'This overview makes the page feel more serious and helps admins read case pressure before diving into details.'}
            </p>
          </div>
        </div>
        <div className="premium-subpanel space-y-1">
          <p className="metric-label">{language === 'vi' ? 'Open cases' : 'Open cases'}</p>
          <p className="metric-value">{activeDisputes.length}</p>
          <p className="text-xs text-muted-foreground">{language === 'vi' ? 'dang can ra quyet dinh hoac can them bang chung' : 'still waiting on judgment or additional evidence'}</p>
        </div>
        <div className="premium-subpanel space-y-1">
          <p className="metric-label">{language === 'vi' ? 'Resolved log' : 'Resolved log'}</p>
          <p className="metric-value">{resolvedDisputes.length}</p>
          <p className="text-xs text-muted-foreground">{language === 'vi' ? 'decision trail de audit va doi soat' : 'decision trail available for audit and review'}</p>
        </div>
        <div className="premium-subpanel space-y-1">
          <p className="metric-label">{language === 'vi' ? 'High-value focus' : 'High-value focus'}</p>
          <p className="metric-value">{activeDisputes.filter((d) => d.listing.price >= 30000000).length}</p>
          <p className="text-xs text-muted-foreground">{language === 'vi' ? 'case anh huong manh den perceived trust cua demo' : 'cases with outsized impact on perceived marketplace trust'}</p>
        </div>
      </section>
