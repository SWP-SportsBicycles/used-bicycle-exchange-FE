      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          {language === 'vi' 
            ...
            : 'Fill in the details about your bicycle'}
        </p>
      </div>

      <section className="premium-panel mb-8 grid gap-4 border border-[#407F3E]/15 bg-gradient-to-br from-white via-[#f6fbf5] to-[#eef7eb] p-5 lg:grid-cols-[1.3fr_repeat(3,minmax(0,1fr))]">
        <div className="space-y-3">
          <span className="eyebrow-chip">{language === 'vi' ? 'Listing onboarding' : 'Listing onboarding'}</span>
          <div>
            <h2 className="text-xl font-semibold tracking-tight" style={{ fontFamily: 'var(--font-archivo)' }}>
              {language === 'vi' ? 'Tao tin dang dep, ro rang va san sang cho review.' : 'Build a listing that looks sharp and review-ready.'}
            </h2>
            <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
              {language === 'vi'
                ? 'Flow nay uu tien chat luong hinh anh, thong so xac minh va framing gia de listing demo nhin nhu da qua mot quy trinh kiem duyet that.'
                : 'This flow prioritizes photo quality, verifiable specs, and pricing framing so the listing feels like it has passed a serious marketplace process.'}
            </p>
          </div>
        </div>

        <div className="premium-subpanel space-y-1">
          <p className="metric-label">{language === 'vi' ? 'Proof assets' : 'Proof assets'}</p>
          <p className="metric-value">3</p>
          <p className="text-xs text-muted-foreground">
            {language === 'vi' ? 'anh tong the, serial, va groupset la bo ba quan trong nhat' : 'overview, serial, and groupset shots are the most important proof trio'}
          </p>
        </div>

        <div className="premium-subpanel space-y-1">
          <p className="metric-label">{language === 'vi' ? 'Review target' : 'Review target'}</p>
          <p className="metric-value">24h</p>
          <p className="text-xs text-muted-foreground">
            {language === 'vi' ? 'microcopy va state duoc framing de seller hieu ro buoc duyet' : 'copy and states are framed so sellers understand the approval step clearly'}
          </p>
        </div>

        <div className="premium-subpanel space-y-1">
          <p className="metric-label">{language === 'vi' ? 'Quality cue' : 'Quality cue'}</p>
          <p className="metric-value">A-</p>
          <p className="text-xs text-muted-foreground">
            {language === 'vi' ? 'listing co du lieu day, anh sach va serial ro se trong premium hon' : 'full data, clean photos, and visible serial proof make the demo feel more premium'}
          </p>
        </div>
      </section>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {formData.images.map((img, index) => (
                      <div key={index} className="relative aspect-square rounded-lg overflow-hidden bg-muted">
                        <img src={img} alt={`Bike ${index + 1}`} className="h-full w-full object-cover" />
                        <Button
                          variant="destructive"
                          size="icon"
                ...
                <Card className="bg-muted/50">
                  <CardHeader>
                    <CardTitle className="text-base">
                      {language === 'vi' ? 'Tóm Tắt Tin Đăng' : 'Listing Summary'}
                    </CardTitle>
                  </CardHeader>
