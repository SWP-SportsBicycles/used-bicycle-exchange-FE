        ...
        <section className="mb-8 grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="premium-panel p-5">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="eyebrow-chip">Saved decision set</div>
                <h2 className="mt-3 text-2xl font-bold tracking-[-0.03em] text-foreground">
                  Wishlist now feels like a serious shortlist, not a bookmark dump
                </h2>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  This section is framed for comparison and purchase intent, so saved bikes feel closer
                  to a buying decision than a casual favorites list.
                </p>
              </div>
              <Badge className="rounded-full bg-primary px-3 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-primary-foreground">
                {wishlistItems.length} saved bikes
              </Badge>
            </div>
          </div>
            ...
            {[
              {
                ...
                value: wishlistItems.length === 0 ? 'No active shortlist' : `${wishlistItems.length} bikes under consideration`,
                sub: 'Makes intent feel more commercial and deliberate.',
              },
              {
                icon: ShoppingCart,
                label: 'Next action',
                value: wishlistItems.length > 0 ? 'Ready to compare or deposit' : 'Browse and save candidates',
                sub: 'Pushes the user toward a meaningful next step.',
              },
              {
                icon: ExternalLink,
                label: 'Role of this page',
                value: 'Pre-purchase decision board',
                sub: 'Frames wishlist as part of the funnel, not a side page.',
              },
