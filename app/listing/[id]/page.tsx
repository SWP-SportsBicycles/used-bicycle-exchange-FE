...
export default function ListingDetailPage({ params }: PageProps) {
  ...
  if (isLoadingListing) {
    return (
      <DetailPageLoading
        ...
        contentClassName="py-6 lg:px-6"
        breadcrumbSkeletonClassName="mb-6 h-5 w-44 animate-pulse rounded bg-muted"
        showLeadSkeleton={false}
        gridClassName="grid gap-8 lg:grid-cols-2"
        primarySkeleton={<div className="aspect-[4/3] animate-pulse rounded-xl bg-muted" />}
        secondarySkeleton={
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          >
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-2 text-muted-foreground flex-wrap">
                ...
                <span className="text-border">|</span>
                <span className="viewer-signal ml-1">
            ...
            <div id="decision-support-strip" className="grid gap-3 sm:grid-cols-3">
              {[
                {
                  label: 'Trust state',
                  value: listing.isVeloSafeVerified ? 'Inspection-ready listing' : 'Private seller listing',
                  sub: listing.isVeloSafeVerified ? 'VeloSafe proof visible above the payment step' : 'Needs stronger proof before final commitment',
                  icon: ShieldCheck,
                },
                {
                  label: 'Deal signal',
                  value: listing.price < marketPrice ? `Below market by ${formatVND(marketPrice - listing.price)}` : 'Priced close to market',
                  sub: 'Helps the buyer decide if this is a fast decision or a compare-more listing.',
                  icon: Star,
                },
                {
                  label: 'Next move',
                  value: isSoftReserved ? 'Ready for inspection deposit' : 'Start with soft reserve',
                  sub: isSoftReserved ? 'Buyer can unlock the inspection workflow now.' : 'Reserve first to reveal handoff details and hold the bike.',
                  icon: Eye,
                },
            ...
            <PriceInsight currentPrice={listing.price} marketPrice={marketPrice} />
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-border/60 bg-background/95 backdrop-blur-md p-4 lg:hidden pb-safe">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-xs text-muted-foreground">Giá bán</p>
            <p className="text-xl font-bold text-primary">{formatVND(listing.price)}</p>
          </div>
          <Button 
            className="flex-1 max-w-[200px]"
            onClick={() => {
              setDepositFlowType(isSoftReserved ? 'inspection_deposit' : 'soft_reserve')
              setIsDepositModalOpen(true)
            }}
          >
            {isSoftReserved ? 'Đặt cọc kiểm định' : 'Soft Reserve'}
          </Button>
        </div>
      </div>
    </div>
