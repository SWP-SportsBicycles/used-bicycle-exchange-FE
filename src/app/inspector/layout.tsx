...
export default function InspectorLayout({ children }: { children: ReactNode }) {
  ...
  const reportCount = (reportsQuery.data ?? []).filter((report) => report.status === 'Pending').length
  ...
  const sidebarItemsWithBadge = sidebarItems.map((item) =>
    item.href === '/inspector/assigned'
      ? pendingCount > 0
        ...
        : item
      : item.href === '/inspector/report'
        ? reportCount > 0
          ? { ...item, badge: reportCount, badgeTone: 'alert' as const }
          : item
        : item
  )
