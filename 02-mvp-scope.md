...
| **PII Seller** | SĐT + địa chỉ cụ thể chỉ sau **Đặt cọc đã xác nhận** (không chỉ sau đăng nhập). |

## 1. Vai trò trong v1

| Vai trò | Trong MVP? | Ghi chú |
|---------|-------------|---------|
| **Guest** | Có | Xem danh sách + chi tiết; **không** thấy SĐT & địa chỉ đầy đủ của Seller cho đến khi (Buyer đã đăng nhập và) có **cọc xác nhận** trên đơn liên quan — xem 03-B.2. |
| **Buyer** | Có | Đăng ký/đăng nhập, lọc/tìm trong 3 TP, wishlist, đặt cọc, luồng kiểm định chuẩn, đánh giá sau `completed`. |
...
|----------|-----------------|
| **Offline (chuyển khoản + xác nhận)** | **Có — mặc định v1** nếu chưa có PSP; logic phân bổ cọc 50/50 vẫn ghi nhận trên sổ nội bộ / ví. |
| **Escrow / cổng thanh toán** | Bật dần theo [SRS.md](./SRS.md) mục 6; **điều kiện hoàn tất đơn** (mục 4 file 01) cần trạng thái **giao thành công** từ logistics hoặc xác nhận tay. |
...
- Đặt cọc `min(10%, 2tr)`; sau cọc xác nhận mới lộ SĐT/địa chỉ đầy đủ.
...
2. Buyer đặt cọc xác nhận → thấy PII → một vòng **kiểm định tại nhà** → báo cáo trên app.
