...
|-----------|------------|
| **Đặt cọc** | Buyer đặt **cọc cố định** để **giữ chỗ** và **kích hoạt dịch vụ kiểm định** theo kịch bản chuẩn mục 5. |
| **Số tiền cọc** | `min(10% × giá niêm yết, 2.000.000 VND)` — làm tròn theo đơn vị nhỏ nhất hệ thống (vd 1.000đ). |
...
2. `reserved` — cọc đã xác nhận, đang trong luồng kiểm định / mua.
...
## 3. Cọc — thanh toán phần còn lại (gợi ý vận hành)

| Tham số | Giá trị (đã chốt / gợi ý) |
|---------|----------------------------|
| Công thức cọc | **10%** giá xe, **tối đa 2.000.000đ** |
...
Đơn chuyển **`completed`** khi **một trong hai** điều kiện xảy ra (cái nào đến trước tính theo cấu hình hệ thống — mặc định dưới đây):
...
## 5. Kịch bản kiểm định chuẩn (sau cọc)
...
1. Buyer **đặt cọc** (đã xác nhận) → kích hoạt lịch **Inspector đến nhà Seller** kiểm tra.  
...
4. **Hủy với hoàn cọc 100%:** khi báo cáo kết luận **lỗi nặng hơn mô tả** (vd nứt khung, hàng giả, sai lệch nghiêm trọng so với tin đăng) — Admin có thể override nếu tranh chấp.
...
- **Điều kiện:** Không chứng minh được **lỗi từ xe / Seller** (theo tiêu chí Admin).  
- **Hậu quả:** **Mất toàn bộ cọc** — phân bổ: **50%** cho **Seller**, **50%** cho **Sàn** (phí vận hành).

### 7.2 Seller hủy kèo

- **Hậu quả:** **Hoàn 100% cọc** cho Buyer.  
...
- Nếu Buyer **không thanh toán phần còn lại** trong SLA sau khi bấm **Tiếp tục mua**: mặc định coi là **Buyer hủy kèo** — áp dụng **mục 7.1** (50% Seller / 50% Sàn), trừ khi Admin bật chính sách ngoại lệ (ghi trong điều khoản).
...
- **Phân công Inspector:** trong **24 giờ** làm việc sau cọc xác nhận.  
